...
description: Parse a user-uploaded resume into a source-grounded candidate profile, screen state-owned enterprise and public-institution jobs from a Feishu table, preserve the fixed six-sheet Excel layout, and compare daily changes. Use when the user asks to screen央国企/事业单位 jobs, prioritize no-written-exam roles, regenerate the established job workbook, or monitor the Feishu recruitment table. Supports PDF, DOCX, TXT, and Markdown resumes; never use a hardcoded candidate profile.
---
...
2. Open the Feishu table and record its latest modification time.
3. Filter `央国企` and `事业单位`; preserve the rule `/ = 无笔试` and `有笔试 = 有笔试`.
4. Derive region, cohort, education, major, and role filters from `resume_profile.json` plus explicit current-request preferences.
...
Return the generated workbook link, the saved profile JSON link when available, the unchanged job counts, the priority-job table, and daily differences when `DAILY_MODE=true`. If execution stops for a resume problem, return only the actionable resume error and the required next step; do not emit a misleading job result.
