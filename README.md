# 央国企与事业单位岗位智能筛选 Skill

这是一个面向求职者的 ChatGPT/Codex Skill。它会先读取用户本次上传的简历，将简历整理成带有原文证据的候选人画像，再从飞书招聘表中筛选央国企和事业单位岗位，优先找出“无笔试、仍可投、与简历匹配”的机会，并生成结构固定的六工作表 Excel 文件。
...
2. 在同一条消息中上传一份简历。
3. 提供有权访问的飞书招聘表链接。
4. 明确本次是否需要每日对比，以及是否有地区、城市、岗位方向等补充偏好。
5. 在消息中使用 `$screen-state-owned-jobs` 调用 Skill。

可以直接复制下面的提示词：

```text
请使用 $screen-state-owned-jobs 读取我本次上传的简历，并筛选下面飞书表中的央国企和事业单位岗位：
...
```text
请使用 $screen-state-owned-jobs，根据我刚上传的简历动态建立候选人画像，然后筛选这张飞书招聘表：<链接>。
...
- 本 Skill 提供岗位整理和决策辅助，不保证录取，也不能替代招聘单位的最终资格审查。
