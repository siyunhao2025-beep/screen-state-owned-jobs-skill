...
这是一个依托两张互补飞书招聘表、面向求职者的 ChatGPT/Codex Skill。它会先读取用户本次上传的简历，将简历整理成带有原文证据的候选人画像，再按用户选择读取一张表或同时读取两张表，筛选央国企、事业单位、民企、外企、上市公司、教育、医疗、高校及人才引进等岗位。默认采用“两表互补”；用户也可以限定数据源、分类或单位性质。Skill 会优先找出“无笔试、仍可投、与简历匹配”的机会，并生成结构固定的六工作表 Excel 文件。
...
3. 也可以直接点击：**[下载可导入的 Skill 安装包](https://github.com/siyunhao2025-beep/job-is-coming-good-luck/raw/refs/heads/main/screen-state-owned-jobs-import-v2.zip)**；
